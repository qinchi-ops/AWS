...
INGRESS_PORTS = os.getenv('PORTS', "443").split(",")
SERVICE = os.getenv('SERVICE', "CLOUDFRONT")
NAME = os.getenv('PREFIX_NAME', "AUTOUPDATE")
VPC_ID = os.getenv('VPC_ID', "vpc-00bf73867ddbd506f")
REGION = os.getenv('REGION', "ap-east-1")
NRANGES = 0
NRULES = 60
...
def update_security_group(client, rangeToUpdate, new_ranges):
    ...
    logging.info(
        ("Total number of rules to be revoked in all the security groups are " + str(count * len(INGRESS_PORTS))))
...
def dynamic_rule_add(client, final_add, to_add, total):
    ...
    for each_grp in final_add:
        ...
        logging.info(("Number of rules can security group " + each_grp + " accomodate: " + str(
        ...
        for each_proto in INGRESS_PORTS:
            ...
            logging.info(("Modified " + str(len(to_add[
                                                0:num_accomodate])) + " rules on security group " + each_grp + " for the port " + each_proto))
...
def revoke_permissions(client, group, to_revoke):
    ...
    for each_proto in INGRESS_PORTS:
        ...
        logging.info(
            ("Revoked " + str(len(to_revoke)) + " rules from the security group " + group + " with port " + each_proto))
